...
std::vector<long long int> pos; // store the positive exponential value of finite field
std::vector<long long int> binv; // store b*a^(-k), b*a^(-2k)..., b*a^(-rk) rk>p
...
int main(int argc, char const *argv[])
{
	...
	while ( !input.eof() )
	{
		...
		g = stol(result[1]);
		x = stol(result[2]);
		output << p << "," << g << "," << x << "," << BabyGiant(p, g, x) << endl;
	}
